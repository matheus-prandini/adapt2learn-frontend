...
export default function GameMetricsBuilder({ games, range }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const data = await apiJson(`/events/game/${gameId}/list`, undefined, 'Erro ao carregar eventos do jogo.')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const data = await apiJson(`/events/game/${gameId}/fields?${qs}`, undefined, 'Erro ao carregar campos do evento.')
  ...
  const compute = async () => {
    ...
    setComputing(true)
    try {
      const data = await apiJson('/events/game/custom', {
        method: 'POST',
        ...jsonBody({
          game_id: gameId,
    ...
    <div className="a2l-stack" style={{ gap: 24 }}>
      <div className="a2l-grid a2l-grid--2">
        ...
        <Field label="Evento" hint={!gameId ? 'Escolha o jogo primeiro.' : undefined}>
          <select value={eventType} onChange={e => onEventChange(e.target.value)} disabled={!gameId || events.length === 0}>
            <option value="">{gameId && events.length === 0 ? 'Nenhum evento registrado' : 'Selecione um evento'}</option>
            {events.map(ev => <option key={ev} value={ev}>{ev}</option>)}
          </select>
        </Field>

        <Field label="User ID" optional hint="Restringe o cálculo a um único aluno.">
          <input value={userId} onChange={e => setUserId(e.target.value)} placeholder="uid do aluno" />
        </Field>
      </div>
      ...
      {eventType && (
        <section>
          <SectionTitle
            action={
              <Button variant="secondary" size="sm" icon={<LuPlus size={15} />} onClick={() => setRows(prev => [...prev, newField()])}>
                Adicionar campo
              </Button>
            }
          >
          ...
          {rows.length === 0 ? (
            <Card quiet>
              <EmptyState title="Nenhum campo ainda" description="Adicione um campo do payload e escolha a operação." />
            </Card>
          ) : (
            <div className="a2l-stack" style={{ gap: 10 }}>
              {rows.map(r => (
                <div key={r.id} className="a2l-filters" style={{ marginBottom: 0 }}>
                  <select className="a2l-input" value={r.field} onChange={e => updateRow(r.id, { field: e.target.value })}>
                    <option value="">Selecione campo</option>
                    {fields.map(f => <option key={f} value={f}>{f}</option>)}
                  </select>
                  <select className="a2l-input" value={r.operation} onChange={e => updateRow(r.id, { operation: e.target.value })}>
                  ...
                  <input
                    ...
                    onChange={e => updateRow(r.id, { condition: e.target.value ? { value: e.target.value } : null })}
                  />
                  <Button variant="ghost" size="sm" onClick={() => removeRow(r.id)} title="Remover campo" aria-label="Remover campo">
                    <LuCircleX size={16} />
                  </Button>
                </div>
              ))}
