...
export default function Register() {
  ...
  async function handleGoogleRegister() {
    ...
    try {
      ...
      try {
        await apiJson('/signup-google', { method: 'POST', ...jsonBody(buildSchoolPayload()) },
  ...
  const schoolFields = (
    <>
      ...
      <div className="a2l-grid a2l-grid--2">
        ...
        >
          <select required={role === 'student'} value={grade} onChange={e => setGrade(e.target.value)}>
            <option value="">Selecione…</option>
      ...
      <Field label={<><LuSchool size={14} /> Escola</>} required hint="Escolha na lista ou digite o nome da sua escola.">
        <input list="schools" required placeholder="Nome da escola" value={school} onChange={e => setSchool(e.target.value)} />
      </Field>
      <datalist id="schools">
        ...
        {method === 'google' ? (
          ...
          <form onSubmit={handleEmailRegister} className="a2l-stack" style={{ gap: 16 }}>
            <Field label={<><LuUser size={14} /> Nome completo</>} required>
              <input type="text" required autoComplete="name" placeholder="Seu nome" value={name} onChange={e => setName(e.target.value)} />
            </Field>

            <Field label={<><LuMail size={14} /> E-mail</>} required>
              <input type="email" required autoComplete="email" placeholder="voce@escola.com" value={email} onChange={e => setEmail(e.target.value)} />
            </Field>
            ...
            <div className="a2l-grid a2l-grid--2">
              ...
              >
                <input type="password" required autoComplete="new-password" placeholder="••••••••" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
              </Field>
            </div>
